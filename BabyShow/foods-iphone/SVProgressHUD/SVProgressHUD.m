...
BOOL isSVProgressHUDFullScreen = NO;

+ (void)setIsSVProgressHUDFullScreen:(BOOL)isFullScreen{
    isSVProgressHUDFullScreen = isFullScreen;
}
...
- (void)positionHUD:(NSNotification*)notification {
    ...
    //kaiqing -42
    if (isSVProgressHUDFullScreen == NO) {
        posY -= 42;
    }else{
        isSVProgressHUDFullScreen = NO;
    }
	...
	[UIView animateWithDuration:0.15
					 ...
					 completion:^(BOOL finished){ 
                         if(sharedView.alpha == 0) {
                             ...
                             // Refresh key if key is nil
                             UIWindow* keyWindow = [UIApplication sharedApplication].keyWindow;
                             if (!keyWindow) {
                                 NSLog(@"Refresh key window");
...
- (UIWindow *)overlayWindow {
    if(!overlayWindow) {
        ...
        //kaiqing 42
        if (isSVProgressHUDFullScreen == NO) {
            CGRect frm = overlayWindow.frame;
            frm.origin.y = 42;
            frm.size.height -=42;
            overlayWindow.frame = frm;
        }
